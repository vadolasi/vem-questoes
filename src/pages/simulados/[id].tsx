...
export default function Questoes() {
  ...
  useEffect(() => {
    if (mode === "raiz") {
      timer.restart(
        new Date(Date.now() + (data?.simulado.totalMinutes || 0) * 60000),
        false
      );
    }
  }, [data, mode]);
  ...
  function handleChangeQuestionByInput() {
    if (data?.simulado.questions.length) {
      if (
        questionInput >= 1 &&
        questionInput <= data?.simulado.questions.length
      ) {
          ...
          <Navigation>
            ...
            <GoTo>
              <input
                ...
                min={1}
                value={questionInput}
                onChange={(e: any) => setQuestionInput(Number(e.target.value))}
              />
              <span>Ir Para</span>
          ...
          <QuestionButtons>
            <div className="resposta">
              <button
                ...
                disabled={fetching || !counter.isRunning}
              >
